...
export default function StudyPlanDisplay({ studyPlan, onSave, onEdit }: StudyPlanDisplayProps) {
  ...
  const getTaskIcon = (type: string) => {
    switch (type) {
      case 'study': return <MenuBook sx={{ color: '#083c70ff' }} />
      case 'review': return <TrendingUp sx={{ color: '#4caf50' }} />
      case 'practice': return <Edit sx={{ color: '#ff9800' }} />
      case 'break': return <Timer sx={{ color: '#9e9e9e' }} />
      default: return <Schedule sx={{ color: '#083c70ff' }} />
    }
  }

  const getTaskColor = (type: string) => {
    switch (type) {
      case 'study': return '#083c70ff'
      case 'review': return '#4caf50'
      case 'practice': return '#ff9800'
      case 'break': return '#9e9e9e'
      default: return '#083c70ff'
    }
  }

  const completedTasks = planData.schedule.reduce((total, day) => 
    total + day.tasks.filter(task => task.completed).length, 0
  )
  const totalTasks = planData.schedule.reduce((total, day) => total + day.tasks.length, 0)
...
${day.tasks.map(task => `• ${task.topic} (${task.duration}h) - ${task.type}`).join('\n')}
`).join('\n')}
              ...
              <AccordionSummary expandIcon={<ExpandMore />}>
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', mr: 2 }}>
                  <Box>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      {day.date} • {day.totalHours} hours • {day.tasks.filter(t => t.completed).length}/{day.tasks.length} completed
                    </Typography>
                  </Box>
                  <LinearProgress
                    variant="determinate"
                    value={(day.tasks.filter(t => t.completed).length / day.tasks.length) * 100}
                    sx={{ width: 100, mr: 2 }}
                  />
                </Box>
              </AccordionSummary>
                        ...
                        <CardContent sx={{ p: 2 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            <IconButton
                              ...
                              sx={{ 
                                color: task.completed ? '#4caf50' : 'action.disabled',
          ...
          <Grid container spacing={2}>
            {planData.tips.map((tip, index) => (
              <Grid size={{ xs: 12 }} key={index}>
                <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2, p: 2, bgcolor: '#f8f9fa', borderRadius: 2 }}>
                  <Typography variant="h6" sx={{ color: '#083c70ff', minWidth: 24 }}>
                    {index + 1}.
                  </Typography>
