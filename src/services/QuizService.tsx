...
const transformQuestions = (questions: Question[]) => {
  return questions.map(question => {
    const baseQuestion = {
      ...
      correctAnswer: '',
      options: undefined as Array<{ optionText: string; isCorrect: boolean }> | undefined
    };
    ...
    if (question.type === 'mcq' && question.options) {
      ...
      baseQuestion.correctAnswer = question.correctAnswer === true ? 'True' : 'False';
...
export const QuizService = {
  ...
  async getQuizDetails(quizId: string): Promise<any> {
    try {
      ...
      if (error.response?.data?.message) {
        ...
        throw new Error('Failed to fetch quiz details. Please try again.');
      }
    }
  },
  ...
  async startQuizAttempt(quizId: string): Promise<any> {
    try {
      const response = await axios.post('http://localhost:3000/quiz/attempt/start', 
        { quizId },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if (error.response?.data?.message) {
        ...
        throw new Error('Failed to start quiz attempt. Please try again.');
      }
    }
  },

  async saveQuizAnswer(attemptId: string, questionId: string, answer: string): Promise<any> {
    try {
      const response = await axios.post('http://localhost:3000/quiz/attempt/answer',
  ...
  async completeQuizAttempt(attemptId: string): Promise<any> {
    try {
      const response = await axios.put('http://localhost:3000/quiz/attempt/complete',
        { attemptId },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if (error.response?.data?.message) {
        ...
        throw new Error('Failed to complete quiz. Please try again.');
      }
    }
  },
